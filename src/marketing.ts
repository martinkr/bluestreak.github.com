import "./styles.css";
import { applyShikiToCodeBlocks } from "./code-highlighting";
...
const dePack: LocaleKeywordPack = {
  ...
  patterns: [/\bDE\d{2}[0-9]{18}\b/g], // add German IBAN
};
...
const installBtn = app.querySelector<HTMLButtonElement>(".install-copy-btn");
if (installBtn) {
  const ICON_COPY = installBtn.innerHTML;
  ...
  installBtn.addEventListener("click", () => {
    navigator.clipboard.writeText("npm install bluestreak").then(() => {
      installBtn.innerHTML = ICON_CHECK;
      installBtn.classList.add("install-copy-btn--copied");
      installBtn.title = "Copied!";
      setTimeout(() => {
        installBtn.innerHTML = ICON_COPY;
        installBtn.classList.remove("install-copy-btn--copied");
        installBtn.title = "Copy";
      }, 2000);
    });
  });
}

void applyShikiToCodeBlocks(app);
